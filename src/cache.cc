...
float MAX_PMC = 0;
uint64_t MAX_LLC_LATENCY = 0;

/*
uint64_t pmc_0_99 = 0;
uint64_t pmc_100_199 = 0;
uint64_t pmc_200_299 = 0;
uint64_t pmc_300_399 = 0;
uint64_t pmc_400_499 = 0;
uint64_t pmc_500_599 = 0;
uint64_t pmc_600_699 = 0;
uint64_t pmc_700_799 = 0;
uint64_t pmc_800_899 = 0;
uint64_t pmc_900_999 = 0;
uint64_t pmc_1000_1099 = 0;
uint64_t pmc_1100_1199 = 0;
uint64_t pmc_1200_1299 = 0;
uint64_t pmc_1300_above = 0;
uint64_t LLC_MSHR = 0;
*/
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
	    ...
	    if(warmup_complete[fill_cpu])
	      {
		      uint64_t pure_miss_reduced_tag = 0;
              uint64_t pure_miss_tag = 0;  
              uint64_t current_miss_latency = (current_core_cycle[fill_cpu] - MSHR.entry[mshr_index].cycle_enqueued);	
		               total_miss_latency += current_miss_latency;
/*
	if(cache_type == IS_LLC && current_miss_latency > MAX_LLC_LATENCY)
	{
		cout << " current_miss_latency: " << current_miss_latency  << endl;		
		MAX_LLC_LATENCY = current_miss_latency;
	}                
*/
               uint64_t start = MSHR.entry[mshr_index].cycle_enqueued;
               uint64_t end = current_core_cycle[fill_cpu];
               
               for (uint64_t now = MSHR.entry[mshr_index].cycle_enqueued; now <= current_core_cycle[fill_cpu]; now++)
               {
                    active_cycles.insert(now);
                    active_miss_cycles.insert(now);  
                    if(active_cycles.size() >= SET_MAX && active_miss_cycles.size() >= SET_MAX)
                    {
                        std::set<uint64_t>::iterator it1 = active_cycles.begin();
                        active_cycles.erase(it1);
                        std::set<uint64_t>::iterator it2 = active_miss_cycles.begin();
                        active_miss_cycles.erase(it2);
                        total_active_cycles++;
                        total_active_miss_cycles++;    
                    }
		    
		    MSHR.entry[mshr_index].mlp = MSHR.entry[mshr_index].mlp + 1.0/MSHR.occupancy;
			
                    if(active_hit_cycles.find(now) == active_hit_cycles.end())
                    {
                        pure_miss_tag = 1;
//			if (cache_type == IS_LLC)
//			{
				MSHR.entry[mshr_index].pmc = MSHR.entry[mshr_index].pmc + 1.0/MSHR.occupancy;
//                    	}
		    }   
                } 
		
		block[set][way].pmc = MSHR.entry[mshr_index].pmc;
		block[set][way].mlp = MSHR.entry[mshr_index].mlp;
            // pure miss
                if (pure_miss_tag == 1)
                {
                    sim_pure_miss[fill_cpu][MSHR.entry[mshr_index].type]++;
                }
                else
                {   // pure miss reduced by prefetcher
                    while(start <= end)
                    {
                        if(active_hit_cycles_wo_prefetch.find(start) == active_hit_cycles_wo_prefetch.end())
                        {
                            pure_miss_reduced_tag = 1;
                            break;
                        }
                        start = start + 1;
                    }
                }
                if(pure_miss_reduced_tag == 1)
                {
                    sim_pure_miss_reduced_by_prefetch[fill_cpu][MSHR.entry[mshr_index].type]++;
                }
	      }


	switch ((int)MSHR.entry[mshr_index].pmc / 50) {
...
/*
	if( warmup_complete[fill_cpu] && cache_type == IS_LLC )
	{
		cout << " PMC: " << MSHR.entry[mshr_index].pmc
		<< " PMC: " << block[set][way].pmc
		<< endl;
	}
*/
...
/*		if( warmup_complete[fill_cpu] && cache_type == IS_LLC )
		{	
			ofstream outFile;
			outFile.open("/home/cc/ETH-ChampSim/data.csv", std::ios::app);	
	    		outFile << " TIME: " << current_core_cycle[fill_cpu] 
			<< " BLOCK_ADDR: " << block[set][way].address
			<< " PMC: " << MSHR.entry[mshr_index].pmc  	
			<< endl;
			outFile.close();
	    	}            
*/
	    
	    MSHR.entry[mshr_index].pmc = 0;
            MSHR.entry[mshr_index].mlp = 0;
	    MSHR.remove_queue(&MSHR.entry[mshr_index]);
            MSHR.num_returned--;
	    MSHR_evit++;

/*
            	if(cache_type == IS_LLC && MSHR.occupancy > 3)
	    	{
			cout << " LLC MSHR SIZE: " << MSHR.SIZE << endl;
...
/*	 
                if(cache_type == IS_LLC && MSHR.entry[mshr_index].pmc > MAX_PMC)
                {
                        cout << " PMC: " << MSHR.entry[mshr_index].pmc  << endl;
                	MAX_PMC = MSHR.entry[mshr_index].pmc;
		} 
*/
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        uint64_t now = WQ.entry[WQ.head].event_cycle;
        uint64_t start = now;
        uint64_t end = current_core_cycle[writeback_cpu];
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            while (now <= current_core_cycle[writeback_cpu] + LATENCY)
            {
                active_cycles.insert(now);
                active_hit_cycles.insert(now);
                if(active_cycles.size() >= SET_MAX && active_hit_cycles.size() >= SET_MAX)
                {
                    std::set<uint64_t>::iterator it1 = active_cycles.begin();
                    active_cycles.erase(it1);
                    std::set<uint64_t>::iterator it2 = active_hit_cycles.begin();
                    active_hit_cycles.erase(it2);
                    total_active_cycles++;
                    total_active_hit_cycles++;
                }                

                // pure miss reduced by prefetcher
                //if(WQ.entry[index].type != 2)
                if(!block[set][way].prefetch)
                {
                    active_hit_cycles_wo_prefetch.insert(now);
                    if(active_hit_cycles_wo_prefetch.size() >= SET_MAX)
                    {
                        std::set<uint64_t>::iterator it1 = active_hit_cycles_wo_prefetch.begin();
                        active_hit_cycles_wo_prefetch.erase(it1);
            ...
            if(pure_miss_reduced_tag == 1)
            {
                sim_pure_miss_reduced_by_prefetch[writeback_cpu][WQ.entry[index].type]++;
            }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
      {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            uint64_t now = RQ.entry[RQ.head].event_cycle;
            uint64_t start = now;
            uint64_t end = current_core_cycle[read_cpu]; 
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                while (now <= current_core_cycle[read_cpu] + LATENCY)
                {
                    active_cycles.insert(now);
                    active_hit_cycles.insert(now);
                    if(active_cycles.size() >= SET_MAX && active_hit_cycles.size() >= SET_MAX)
               ...
               // pure miss reduced by prefetcher
                ...
                if(pure_miss_reduced_tag == 1)
                {
                    sim_pure_miss_reduced_by_prefetch[read_cpu][RQ.entry[index].type]++;
                }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (prefetch_cpu == NUM_CPUS)
      {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {            
            uint64_t now = PQ.entry[PQ.head].event_cycle;
            // pure miss reduced by prefetcher
            uint64_t start = now;
            uint64_t end = current_core_cycle[prefetch_cpu];
            ...
            if (way >= 0) { // prefetch hit
                ...
                //  active_cycles.insert(current_core_cycle[prefetch_cpu]);
                //  active_hit_cycles.insert(current_core_cycle[prefetch_cpu]);
                while (now <= current_core_cycle[prefetch_cpu] + LATENCY)
                {
                    active_cycles.insert(now);
                    active_hit_cycles.insert(now);
                    if(active_cycles.size() >= SET_MAX && active_hit_cycles.size() >= SET_MAX)
                ...
                if(pure_miss_reduced_tag == 1)
                {
                    sim_pure_miss_reduced_by_prefetch[prefetch_cpu][PQ.entry[index].type]++;
                }
...
void CACHE::operate()
{
    //cout << " MSHR SIZE: " << MSHR.SIZE << endl;
    //handle_fill();
    //handle_writeback();
    reads_available_this_cycle = MAX_READ;
    //cout << unsigned(cache_type);    
    //cout << " MAX READ: " << MAX_READ << endl;
    handle_read();
