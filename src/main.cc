...
void record_roi_stats(uint32_t cpu, CACHE *cache)
{
    for (uint32_t i=0; i<NUM_TYPES; i++) {
        ...
        cache->roi_pure_miss[cpu][i] = cache->sim_pure_miss[cpu][i]; // pure miss measurement
        cache->roi_pure_miss_reduced_by_prefetch[cpu][i] = cache->sim_pure_miss_reduced_by_prefetch[cpu][i]; // pure miss reduced by prefetcher 
    }
}

void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    uint64_t TOTAL_ACCESS = 0, TOTAL_HIT = 0, TOTAL_MISS = 0, TOTAL_PURE_MISS = 0, TOTAL_PURE_MISS_REDUCED_BY_PREFETCH = 0;

    for (uint32_t i=0; i<NUM_TYPES; i++) {
        TOTAL_ACCESS += cache->roi_access[cpu][i];
        TOTAL_HIT += cache->roi_hit[cpu][i];
        TOTAL_MISS += cache->roi_miss[cpu][i];
        TOTAL_PURE_MISS += cache->roi_pure_miss[cpu][i];
        TOTAL_PURE_MISS_REDUCED_BY_PREFETCH += cache->roi_pure_miss_reduced_by_prefetch[cpu][i];
    }
    ...
    cout<< "Core_" << cpu << "_" << cache->NAME << "_total_access " << TOTAL_ACCESS << endl
        ...
        << "Core_" << cpu << "_" << cache->NAME << "_miss_rate " << (1.0*TOTAL_MISS)/TOTAL_ACCESS << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_MPKI " << (1.0* TOTAL_MISS*1000)/simulation_instructions << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_demand_miss " << TOTAL_MISS - cache->roi_miss[cpu][2] << endl
        ...
        << "Core_" << cpu << "_" << cache->NAME << "_active_cycles " << cache->total_active_cycles + cache->active_cycles.size() << endl
        << "Core_" << cpu << "_" << cache->NAME << "_active_hit_cycles " << cache->total_active_hit_cycles + cache->active_hit_cycles.size() << endl
        << "Core_" << cpu << "_" << cache->NAME << "_active_miss_cycles " << cache->total_active_miss_cycles + cache->active_miss_cycles.size()<< endl
        << "Core_" << cpu << "_" << cache->NAME << "_active_hit_cycles_wo_prefetch " << cache->total_active_hit_cycles_wo_prefetch + cache->active_hit_cycles_wo_prefetch.size()<< endl
        << "Core_" << cpu << "_" << cache->NAME << "_total_pure_miss " << TOTAL_PURE_MISS << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_pure_miss_rate " << (1.0*TOTAL_PURE_MISS)/TOTAL_ACCESS << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_demand_pure_miss " << TOTAL_PURE_MISS - cache->roi_pure_miss[cpu][2] << endl
        << "Core_" << cpu << "_" << cache->NAME << "_total_miss_pure_miss_diff " << TOTAL_MISS - TOTAL_PURE_MISS << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_demand_miss_pure_miss_diff " << TOTAL_MISS - cache->roi_miss[cpu][2] - (TOTAL_PURE_MISS - cache->roi_pure_miss[cpu][2]) << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_total_pure_miss_reduced_by_prefetch " << TOTAL_PURE_MISS_REDUCED_BY_PREFETCH << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_demand_pure_miss_reduced_by_prefetch " << TOTAL_PURE_MISS_REDUCED_BY_PREFETCH - cache->roi_pure_miss_reduced_by_prefetch[cpu][2] << endl
        << "Core_" << cpu << "_" << cache->NAME << "_hit_miss_overlap_cycles " << cache->active_hit_cycles.size() + cache->total_active_hit_cycles + cache->active_miss_cycles.size() + cache->total_active_miss_cycles - cache->active_cycles.size() - cache->total_active_cycles<< endl
        << "Core_" << cpu << "_" << cache->NAME << "_active_pure_miss_cycles " << (cache->active_miss_cycles.size() + cache->total_active_miss_cycles)  - ((cache->active_hit_cycles.size() + cache->total_active_hit_cycles + cache->active_miss_cycles.size() + cache->total_active_miss_cycles) - cache->active_cycles.size() - cache->total_active_cycles) << endl
        << "Core_" << cpu << "_" << cache->NAME << "_camat " << (1.0*(cache->active_cycles.size() + cache->total_active_cycles )) / TOTAL_ACCESS << endl
        << "Core_" << cpu << "_" << cache->NAME << "_k " << (1.0*(cache->active_miss_cycles.size() + cache->total_active_miss_cycles  - ((cache->active_hit_cycles.size() + cache->total_active_hit_cycles + cache->active_miss_cycles.size() + cache->total_active_miss_cycles) - cache->active_cycles.size() - cache->total_active_cycles))) / (cache->active_miss_cycles.size() + cache->total_active_miss_cycles) << endl
        << "Core_" << cpu << "_" << cache->NAME << "_prefetch_accuracy " << (1.0* cache->pf_useful)/cache->pf_issued << endl
        << "Core_" << cpu << "_" << cache->NAME << "_prefetch_coverage_total " << (1.0* cache->pf_useful)/(cache->pf_useful + TOTAL_MISS) << endl
        << "Core_" << cpu << "_" << cache->NAME << "_prefetch_coverage_demand " << (1.0* cache->pf_useful)/(cache->pf_useful + TOTAL_MISS - cache->roi_miss[cpu][2]) << endl
        << "Core_" << cpu << "_" << cache->NAME << "_prefetch_coverage_total_prefetchhit " << (1.0 * cache->roi_hit[cpu][2]) / (cache->roi_hit[cpu][2] + TOTAL_MISS) << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_prefetch_coverage_demand_prefetchhit " << (1.0 * cache->roi_hit[cpu][2]) / (cache->roi_hit[cpu][2] + TOTAL_MISS - cache->roi_miss[cpu][2]) << endl
	<< "Core_" << cpu << "_" << cache->NAME << "_pure_prefetch_coverage_total " << (1.0*TOTAL_PURE_MISS_REDUCED_BY_PREFETCH) / (TOTAL_PURE_MISS_REDUCED_BY_PREFETCH + TOTAL_PURE_MISS) << endl
        << "Core_" << cpu << "_" << cache->NAME << "_pure_prefetch_coverage_demand " << (1.0*(TOTAL_PURE_MISS_REDUCED_BY_PREFETCH - cache->roi_pure_miss_reduced_by_prefetch[cpu][2])) / (TOTAL_PURE_MISS_REDUCED_BY_PREFETCH - cache->roi_pure_miss_reduced_by_prefetch[cpu][2] + TOTAL_PURE_MISS - cache->roi_pure_miss[cpu][2]) << endl
	<< endl;

	cout << " pmc_0_49: " << cache->pmc_0_49  << endl;
	cout << " pmc_50_99: " << cache->pmc_50_99  << endl;
	cout << " pmc_100_149: " << cache->pmc_100_149  << endl;
	cout << " pmc_150_199: " << cache->pmc_150_199  << endl;
	cout << " pmc_200_249: " << cache->pmc_200_249  << endl;
	cout << " pmc_250_299: " << cache->pmc_250_299  << endl;
	cout << " pmc_300_349: " << cache->pmc_300_349  << endl;
        cout << " pmc_350_above: " << cache->pmc_350_above  << endl;
//	cout << " pmc_700_799: " << cache->pmc_700_799  << endl;
//	cout << " pmc_800_899: " << cache->pmc_800_899  << endl;
//	cout << " pmc_900_999: " << cache->pmc_900_999  << endl;
//	cout << " pmc_1000_1099: " << cache->pmc_1000_1099  << endl;
//	cout << " pmc_1100_1199: " << cache->pmc_1100_1199  << endl;
//	cout << " pmc_1200_1299: " << cache->pmc_1200_1299  << endl;
//	cout << " pmc_1300_above: " << cache->pmc_1300_above  << endl;
	cout << " MSHR_evit: " << cache->MSHR_evit << endl;
	cout << " MISS_total: " << cache->miss_total << endl;
...
void reset_cache_stats(uint32_t cpu, CACHE *cache)
{
    for (uint32_t i=0; i<NUM_TYPES; i++) {
        ...
        cache->sim_pure_miss[cpu][i] = 0; // pure miss measurement
        cache->sim_pure_miss_reduced_by_prefetch[cpu][i] = 0; // pure miss reduced by prefetcher
    }
    ...
    cache->active_cycles.clear();
    cache->active_hit_cycles.clear();
    cache->active_miss_cycles.clear();
    // pure miss reduced by prefetcher
    cache->active_hit_cycles_wo_prefetch.clear();
    
    cache->total_active_cycles = 0;
    cache->total_active_hit_cycles = 0;
    cache->total_active_miss_cycles = 0;
    cache->total_active_hit_cycles_wo_prefetch = 0;
    ...
    cache->pmc_250_299 = 0;
    cache->pmc_300_349 = 0;
    cache->pmc_350_above = 0;
     
/*
    cache->pmc_700_799 = 0;
    cache->pmc_800_899 = 0;
    cache->pmc_900_999 = 0;
    cache->pmc_1000_1099 = 0;
    cache->pmc_1100_1199 = 0;
    cache->pmc_1200_1299 = 0;
    cache->pmc_1300_above = 0;
*/
}
...
int main(int argc, char** argv)
{
	...
	ooo_cpu[i].DTLB.fill_level = FILL_L1;
        ...
        ooo_cpu[i].L1D.MAX_READ = MAX_READ_PER_CYCLE;
...
#endif
/*
        cout << " pmc_0_99: " << pmc_0_99  << endl;
        cout << " pmc_100_199: " << pmc_100_199  << endl;
        cout << " pmc_200_299: " << pmc_200_299  << endl;
        cout << " pmc_300_399: " << pmc_300_399  << endl;
        cout << " pmc_400_499: " << pmc_400_499  << endl;
        cout << " pmc_500_599: " << pmc_500_599  << endl;
        cout << " pmc_600_699: " << pmc_600_699  << endl;
        cout << " pmc_700_799: " << pmc_700_799  << endl;
        cout << " pmc_800_899: " << pmc_800_899  << endl;
        cout << " pmc_900_999: " << pmc_900_999  << endl;
        cout << " pmc_1000_1099: " << pmc_1000_1099  << endl;
        cout << " pmc_1100_1199: " << pmc_1100_1199  << endl;
        cout << " pmc_1200_1299: " << pmc_1200_1299  << endl;
        cout << " pmc_1300_above: " << pmc_1300_above  << endl;


	cout << " llc_mshr: " << LLC_MSHR << endl;
*/
